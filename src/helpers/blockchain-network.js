...
import { boardArray, privateArray } from "./Wallet.js";
...
const contract_address = "0x7ef03d076f39ef49062e81ef5f2882828d9170f1";
...
function getNftContract(ethKey) {
  const ethPrivkey = ethKey;
  const wallet = new ethers.Wallet(ethPrivkey, provider);
  // const signer = wallet.provider.getSigner(wallet.address);
  // const signer = provider.getSigner();
  const nftContract = new ethers.Contract(contract_address, nftabi, wallet);
  return nftContract;
}

export async function initialiseNetwork() {
  const myWallet =
    "b0d16087ffe533fbcd641848d5d80894d0fe40b86344a08d999b78c81c2e0fc8";
  const nftContract = getNftContract(myWallet);
  const owner = await nftContract.ownerOf("3");
  const number = await getIndexOfK(boardArray, owner);
  return {
    owner,
    number,
  };
